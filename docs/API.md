...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/photos/spot/{spotId}` | 某点位的照片列表（`thumbUrl`/`url`/`description`…） |
| GET | `/api/photos/mine` | 当前用户照片（平铺，按拍摄时间倒序） |
| POST | `/api/photos` | 上传照片。multipart：`spotId`、`file`、`description?`、`shotTime?`（`yyyy-MM-dd HH:mm:ss`，来自 EXIF）、`device?`（EXIF Make+Model） |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/app/version` | 版本检查。返回 `{ versionCode, versionName, downloadUrl, notes }`（当前 2 / 1.1.0）。客户端比对本地 versionCode 决定是否弹更新 |
